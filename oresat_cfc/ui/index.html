...
<html lang="en">
  <head>
    <title>OreSat CFC {{version}}</title>
  </head>
  <body>
    <style>
      table {
    ...
    <div id="grid">
      ...
      <div id="gridColumn">
        ...
        <br />
        <h3>TEC (Thermoelectric Cooler) Controller</h3>
        <table>
          <tr>
            <td>Enable Status</td>
    ...
    <script>
      ...
      async function getImageRaw() {
       const url = `http://${window.location.host}/image/raw`;
        return await fetch(url)
          .then(response => response.json())
          .then(data => {
            return data.image;
      ...
      async function openCheck(dialog) {
        if (dialog.open) {
          ...
          const stateStr = STATES[data.camera.status];
          ...
          document.getElementById("newState").selectedIndex = data.camera.status - 1;
          document.getElementById("newCapSave").checked = data.camera.save_captures;
          document.getElementById("newCapDelay").value = data.camera.capture_delay;
          document.getElementById("newCapAmount").value = data.camera.number_to_capture;
          document.getElementById("newIntTime").value = Math.round(data.camera.integration_time);
          document.getElementById("newTecStatus").checked = data.tec.status;
          document.getElementById("newSetpoint").value = data.tec.setpoint;
        } else if (dialog.returnValue === "submit") {
          const newData = {
            "camera": {
              "status": document.getElementById("newState").selectedIndex + 1,
              "capture_delay": parseInt(document.getElementById("newCapDelay").value),
              "number_to_capture": parseInt(document.getElementById("newCapAmount").value),
              "save_captures": document.getElementById("newCapSave").checked,
              "integration_time": parseInt(document.getElementById("newIntTime").value),
            },
            "tec": {
              "status": document.getElementById("newTecStatus").checked,
              "setpoint": parseInt(document.getElementById("newSetpoint").value),
            },
          };
      ...
      async function update() {
        const data = await getData();
        let state = STATES[data.camera.status];

        document.getElementById("cfcState").innerHTML = state;
        document.getElementById("captureDelay").innerHTML = data.camera.capture_delay;
        document.getElementById("captureAmount").innerHTML = data.camera.number_to_capture;
        if (data.camera.save_captures === true) {
        ...
        if (state != "OFF" && data.camera.last_capture_time != lastCaptureTime) {
        ...
        document.getElementById("cameraTemperature").innerHTML = data.camera.temperature;
        document.getElementById("cameraIntegrationTime").innerHTML = Math.round(data.camera.integration_time);

        // update TEC info
        if (data.tec.status === true) {
          document.getElementById("tecStatus").innerHTML = "ENABLED";
        } else {
          document.getElementById("tecStatus").innerHTML = "DISABLED";
        }
        if (data.tec.saturated == true) {
          document.getElementById("tecSaturated").innerHTML = "SATURATED";
        } else {
          document.getElementById("tecSaturated").innerHTML = "NOT_SATURATED";
        }
        document.getElementById("tecSetpoint").innerHTML = data.tec.setpoint;
      }
